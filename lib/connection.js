...
import { gcblHas }                                        from './groupBlacklist.js'
import config                                             from '../config.js'
import logger                                             from './logger.js'
import { clearSessions }                                  from './utils.js'
import { loadPlugins, watchPlugins }                      from './loader.js'
import { handleMessage, handleMentionTrigger, handleAutoTrigger } from './handler.js'
import { askPhoneNumber }                                 from './pairing.js'
import { bindContactStore, loadExcludeConfig }            from './statusRuntime.js'
import { bindContactStoreEvents, loadContactStore, upsertFromJid } from './contactStore.js'
...
export async function createConnection() {
  ...
  // ─── Bind semua contact events ──────────────────────────────
  bindContactStoreEvents(sock)

  // ─── Bind status viewer tracker ──────────────────────────────
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'connecting') {
      ...
      if (
        ...
        try {
          ...
          if (!code) throw new Error('Server tidak mengembalikan pairing code')
    ...
    if (connection === 'open') {
      ...
      // ─── Scan semua grup → index semua participant ke contact store ──
      // Delay 3 detik biar socket dan groupFetchAllParticipating siap
      setTimeout(async () => {
        try {
          ...
          for (const [, meta] of Object.entries(chats)) {
            for (const p of meta.participants || []) {
              ...
              const jid  = p.id.includes(':') ? p.id.split(':')[0] + '@s.whatsapp.net' : p.id
              if (!jid.endsWith('@s.whatsapp.net')) continue
              const pNum = jid.split('@')[0].replace(/[^0-9]/g, '')
          ...
          logger.info(`👥 Scan grup selesai: ${added} participant di-index`)
        } catch (e) {
          logger.warn(`⚠️ Gagal scan grup: ${e.message}`)
        }
      }, 3000)
    }
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Antilink — jalan di semua grup termasuk blacklist
        const blocked = await antilinkMiddleware(sock, msg)
        if (blocked) continue

        // Mention trigger (@bot intro, dsb) — jalan di semua grup termasuk blacklist
        const mentionHandled = await handleMentionTrigger(sock, msg)
        if (mentionHandled) continue

        // Auto-trigger (autoai nama bot) — jalan di semua grup termasuk blacklist
        const autoHandled = await handleAutoTrigger(sock, msg)
        if (autoHandled) continue

        // Group blacklist — blok command biasa
        const groupJid = msg.key.remoteJid
        if (groupJid?.endsWith('@g.us') && gcblHas(groupJid)) continue
...
/**
 * Restart bot — tutup koneksi WA lalu spawn ulang proses via start.js
 * Dipanggil dari plugins/owner.js command .restart
 */
export async function restartBot() {
  logger.info('🔄 Restarting bot...')
  const { spawn } = await import('child_process')
  spawn(process.execPath, ['start.js'], {
    detached: true,
    stdio:    'inherit',
    cwd:      process.cwd(),
  }).unref()
...
export async function startBot() {
  ...
  // Load exclude config untuk status runtime
  await loadExcludeConfig()

  // Load semua plugin
  commandMap = await loadPlugins()

  // Pasang hot reload watcher
  watchPlugins(
    () => commandMap,
    (newMap) => { commandMap = newMap },
    loadExcludeConfig,
  )
  ...
  // Load kontak tersimpan dari disk (sebelum socket dibuat)
  await loadContactStore()
