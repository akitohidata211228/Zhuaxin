  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Group blacklist — bot tidak respon command, tapi antilink sudah jalan di atas
...
export async function restartBot() {
  ...
  // Spawn child process baru sebelum exit agar bot tidak mati permanen
...
export async function startBot() {
  ...
  // Pasang hot reload watcher — auto deteksi perubahan plugin & whitelist
  // tanpa perlu restart bot
  watchPlugins(
    () => commandMap,
    (newMap) => { commandMap = newMap },
    loadExcludeConfig, // reload exclude/privacy config saat data/ berubah
  )
