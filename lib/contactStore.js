...
export function bindContactStoreEvents(sock) {
  ...
  // Saat ada member join/masuk grup → tambah ke store
  sock.ev.on('group-participants.update', ({ participants }) => {
    let changed = false
    for (const jid of participants || []) {
      if (!jid?.endsWith('@s.whatsapp.net')) continue
      if (!contacts.has(jid)) {
        contacts.set(jid, { name: '', notify: '' })
        changed = true
      }
    }
    if (changed) scheduleSave()
...
/**
 * Tambah JID ke contact store tanpa nama (dari scan grup)
 * Tidak akan menimpa entry yang sudah ada namanya
 */
export function upsertFromJid(jid) {
