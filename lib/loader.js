...
export async function loadPlugins() {
  ...
  for (const file of jsFiles) {
    ...
    try {
      ...
      // Daftarkan mention trigger (handler.mentionTrigger = 'intro', dll)
      if (handler.mentionTrigger) {
        if (!global.__mentionTriggerMap) global.__mentionTriggerMap = new Map()
        global.__mentionTriggerMap.set(handler.mentionTrigger.toLowerCase(), handler)
      }

      // Daftarkan auto-trigger untuk plugin yang pakai panggil-nama-bot (autoai)
      if (handler.autoTrigger) {
        if (!global.__autoTriggerPlugins) global.__autoTriggerPlugins = []
        global.__autoTriggerPlugins.push(handler)
      }
  ...
  global.__commandMap = commandMap  // Untuk autoai intent routing
  return commandMap
}
  ...
  for (const file of jsFiles) {
    ...
    try {
      ...
      result.push({
        ...
        commandDesc: handler.commandDesc || {},
        category:    (handler.category || ['general'])[0],
      })
    } catch { /* skip */ }
  }
