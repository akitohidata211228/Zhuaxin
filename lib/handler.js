...
export async function handleMessage(sock, msg, commandMap) {
  ...
  if (!senderIsOwner && !senderIsUnresolved) {
    ...
    if (!allowed) {
      // Kirim notif sekali ke PM bahwa mereka belum terdaftar
      // Hanya di private chat agar tidak spam di grup
      if (!isGroup) {
        await sock.sendMessage(jid, {
          text:
            `⛔ *Kamu belum terdaftar di whitelist bot ini.*\n\n` +
            `Hubungi owner untuk mendaftarkan nomormu.\n` +
            `_Nomor: ${config.ownerNumber?.[0] || 'owner'}_`,
        })
      }
      return
    }
