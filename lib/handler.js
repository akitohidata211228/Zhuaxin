...
export async function handleMessage(sock, msg, commandMap) {
  ...
  if (!senderIsOwner && !senderIsUnresolved) {
    ...
    if (!allowed) {
      // Diam total ke user — tidak reply, tidak reaction
      // Tapi kirim notif ke owner (PM ke nomor pertama di config)
      try {
        const senderNumber = sender.replace(/[^0-9]/g, '')
        ...
        const lastNotif = global.__wlNotifyCache.get(throttleKey) || 0
        if (Date.now() - lastNotif > 10 * 60 * 1000) {
          global.__wlNotifyCache.set(throttleKey, Date.now())
