...
export async function handleMessage(sock, msg, commandMap) {
  ...
  if (!senderIsOwner && !senderIsUnresolved) {
    ...
    if (!allowed) {
      // Kirim notif ke owner (PM ke nomor pertama di config)
      // Format nomor: +628xxx (bukan JID)
      try {
        const senderNumber = sender.replace(/[^0-9]/g, '')
        const ownerJid = `${config.ownerNumber?.[0]}@s.whatsapp.net`
        // Throttle: jangan spam owner — kirim max 1x per nomor per 10 menit
        const throttleKey = `wl_notify_${senderNumber}`
        if (!global.__wlNotifyCache) global.__wlNotifyCache = new Map()
        const alreadyNotified = global.__wlNotifyCache.has(throttleKey)
        if (!alreadyNotified) {
          global.__wlNotifyCache.set(throttleKey, true)
          await sock.sendMessage(ownerJid, {
            text:
              `🔔 *Notifikasi Whitelist*\n\n` +
              `Ada user yang mencoba memakai bot:\n` +
              `📱 *+${senderNumber}*\n\n` +
              `_Tambah ke whitelist: *wl add ${senderNumber}*_`,
          })
        }
      } catch { /* jangan crash kalau notif owner gagal */ }

      // Kalau mode "on" → blok user (diam total, tidak reply)
      // Kalau mode "off" → biarkan lanjut meski belum di whitelist
      const { getWhitelistMode } = await import('./whitelist.js')
